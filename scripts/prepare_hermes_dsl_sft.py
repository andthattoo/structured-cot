        ...
        return [
            {
                ...
                "content": (
                    ...
                    "- If NEXT=final, use PLAN=seq(observe,verify,finish), STATE=ready, RISK=none.\n"
                    "- If NEXT=tool_call, STATE must not be ready, and PLAN should include act.\n"
                    "- Use PLAN=seq(observe,act,verify,repair,finish) only when recovering from a failed/wrong action.\n"
                    "- Use PLAN=seq(observe,verify,finish) only for final answers or pure verification summaries.\n"
                    ...
                    "- Use bad_tool_args only for malformed JSON, invalid parameters, quoting/schema issues, or wrong argument shape.\n"
                    "- Use missing_context when search/read found no useful evidence but did not fail.\n"
                    "- Use repeat_loop when the assistant is repeating a prior failed or low-value action.\n"
                    "- Use premature_final only when the assistant is about to answer/finalize without enough evidence; do not use it with NEXT=final for a proper final answer.\n\n"
                    "Examples:\n"
                    "Previous tool response: {\"success\": false, \"error\": \"Session database not available.\"}\n"
                    "Assistant calls search_files next.\n"
                    "PLAN: seq(observe,act,verify,repair,finish)\n"
                    "STATE: need_fix\n"
                    "RISK: tool_failure\n"
                    "NEXT: tool_call\n\n"
                    "Previous tool response: {\"total_count\": 0}\n"
                    "Assistant calls another search next.\n"
                    "PLAN: seq(observe,act,verify,finish)\n"
                    "STATE: need_context\n"
                    "RISK: missing_context\n"
                    "NEXT: tool_call\n\n"
                    "Previous tool response contains useful file contents.\n"
                    "Assistant reads another relevant file next.\n"
                    "PLAN: seq(observe,act,verify,finish)\n"
                    "STATE: need_context\n"
                    "RISK: none\n"
                    "NEXT: tool_call\n\n"
                    "Assistant gives final answer after successful verification.\n"
